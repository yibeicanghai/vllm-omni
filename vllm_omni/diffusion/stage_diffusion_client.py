...
class StageDiffusionClient(StageClientBase):
    ...
    def _drain_responses(self) -> None:
        ...
        while True:
            ...
            if msg_type == "result":
                ...
                # DTPS: DiT-load query reply from the subprocess.
    ...
    async def get_dit_load_async(self, *, timeout_s: float = 0.05) -> tuple[int, int, list[str], list[str]] | None:
        """Return ``(num_waiting, num_running, waiting_ids, running_ids)``.

        DTPS: the Orchestrator polls each DiT replica's queue depth +
        ...
        try:
            self._request_socket.send(self._encoder.encode({"type": "get_load"}))
        except Exception:
            logger.debug(
