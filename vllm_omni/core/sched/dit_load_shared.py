"""DTPS Module 2 — cross-PROCESS DiT-stage load state (shared memory).
...
class DitLoadSharedState:
    ...
    def _truncate_inplace(waiting_ids: list[str], running_ids: list[str]) -> None:
        ...
        while True:
            ...
            if waiting_ids:
                ...
                logger.warning(
                    "[OmniDTPS] DiT-load ID blob full (%d > %d); dropping "
                    "waiting id %r", size, _MAX_BLOB, dropped,
                )
            elif running_ids:
                dropped = running_ids.pop()
                logger.warning(
                    "[OmniDTPS] DiT-load ID blob full (%d > %d); dropping "
                    "running id %r", size, _MAX_BLOB, dropped,
                )
            else:
                return
    ...
    def unlink(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "[OmniDTPS] shared memory unlink failed for name=%s "
